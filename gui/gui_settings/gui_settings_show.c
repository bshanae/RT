						...
						(t_gui_settings *gui, t_rt_settings *settings)
{
	...
	gtk_toggle_button_set_active(gui->tracing_rt, mod == rt_tracing_mod_rt);
	gtk_toggle_button_set_active(gui->tracing_rm, mod == rt_tracing_mod_rm);
	gui->tracing_block = rt_false;
	gtk_stack_set_visible_child_name(gui->stack,
		mod == rt_tracing_mod_rt ? "rt" : "rm");
