...
int					main(int argc, char **argv)
{
	...
	scene_test_illumination(gui->renderer->data.scene);
	gui->renderer->data.camera->position.z = 20;
	gui->renderer->data.camera->position.y = 10;
...
void				scene_test_limited(t_scene *scene)
{
	...
	object_build(scene_get_space(scene), object_type_limited, "Sphere", "Plane A");
	scene_edit_param(scene, -1, scene_param_name, "Limited", scene_param_end);
}
