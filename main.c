...
void				scene_test_b(t_scene *scene)
{
	...
	object_build(scene_get_space(scene), object_type_sphere, (RT_F4_API){10., 0., -10.}, 3.);
	scene_edit_param(scene, -1, scene_param_material, MATERIAL_LIGHT, scene_param_end);
	scene_edit_param(scene, -1, scene_param_name, "Limit Main", scene_param_end);

	object_build(scene_get_space(scene), object_type_plane, (RT_F4_API){0., 0., 0.}, (RT_F4_API){0., 1., 0.}, plane_limiting_no);
	scene_edit_param(scene, -1, scene_param_material, MATERIAL_WHITE, scene_param_end);
	scene_edit_param(scene, -1, scene_param_name, "Limit Plane", scene_param_end);

	object_build(scene_get_space(scene), object_type_limited, "Limit Main", "Limit Plane");
	scene_edit_param(scene, -1, scene_param_name, "Limited", scene_param_end);
}
