									 ...
									 const unsigned char *stbi_data)
{
	...
	pixel_index = 0;
	size = texture->texture_length[texture->textures_number] * 4;
	while (pixel_index < size)
	{
		...
		pointer->w = (RT_F)stbi_data[pixel_index + 3] / 255;
		pointer++;
		pixel_index += 4;
	}
}
...
void				texture_data_load(t_texture *texture, const char *path)
{
	...
	stbi_data = stbi_load(path, &texture_width, &texture_height, &number_of_channels, 4);
