...
void	parse_ambient(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : AMBIENT_TEXTURE);
	object_build(scene_get_space(((t_cl_renderer*)data)->data.scene), object_type_light_ambient);
	scene_edit_param(((t_cl_renderer*)data)->data.scene, -1, scene_param_material, box.material, scene_param_name, box.name, scene_param_texture, box.texture);
	free(box.val_s1);
	free(box.val_s2);
	free(box.val_s3);
}
...
void	parse_point(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : POINT_TEXTURE);
...
void	parse_direct(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : DIRECT_TEXTURE);
...
void	parse_sphere(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : SPHERE_TEXTURE);
...
void	parse_plane(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : PLANE_TEXTURE);
	...
	object_build(scene_get_space(((t_cl_renderer*)data)->data.scene), object_type_plane, *(RT_F4_API*)&box.v1, 0);
	scene_edit_param(((t_cl_renderer*)data)->data.scene, -1, scene_param_material, box.material, scene_param_name, box.name, scene_param_texture, box.texture);
	free(box.val_v1);
	free(box.val_v2);
	free(box.val_s1);
...
void	parse_cone(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : CONE_TEXTURE);
...
void	parse_cylinder(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : CYLINDER_TEXTURE);
...
void	parse_box(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : BOX_TEXTURE);
...
void	parse_paraboloid(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.val_f1 = get_float_in_object(json, tokens, "radius");
	box.texture = (box.val_s2 ? strdup(box.val_s3) : PARABOLOID_TEXTURE);
...
void	parse_moebius(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.val_f1 = get_float_in_object(json, tokens, "radius");
	box.texture = (box.val_s2 ? strdup(box.val_s3) : MOEBIUS_TEXTURE);
...
void	parse_torus(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.val_f1 = get_float_in_object(json, tokens, "radius");
	box.texture = (box.val_s2 ? strdup(box.val_s3) : TORUS_TEXTURE);
	box.val_f2 = get_float_in_object(json, tokens, "width");
...
void	parse_mandelbulb(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : MANDELBULB_TEXTURE);
...
void	parse_julia(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.val_f1 = get_float_in_object(json, tokens, "iterations");
	box.texture = (box.val_s2 ? strdup(box.val_s3) : JULIA_TEXTURE);
...
void	parse_csg(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.texture = (box.val_s2 ? strdup(box.val_s3) : CSG_TEXTURE);
...
void	parse_explosion(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.val_v1 = get_vector_in_object(json, tokens, "position");
	box.texture = (box.val_s2 ? strdup(box.val_s3) : EXPLOSION_TEXTURE);
	box.val_f1 = get_float_in_object(json, tokens, "radius");
...
void	parse_perfcube(void *data, char *json, jsmntok_t *tokens)
{
	...
	box.val_v1 = get_vector_in_object(json, tokens, "position");
	box.texture = (box.val_s2 ? strdup(box.val_s3) : PCUBE_TEXTURE);
